            ...
            try {
                ...
                if (res.ok) {
                    ...
                    list.innerHTML = members.map(m => {
                        ...
                        return renderWorkspaceMemberCard(m);
                        return `
        ...
        function renderWorkspaceMemberCard(member) {
            const user = member.user || {};
            const userId = member.userId || user.id;
            const userName = member.userName || user.name || (member.userEmail ? member.userEmail.split('@')[0] : 'Member');
            const userEmail = member.userEmail || user.email || '';
            const role = member.role || 'VIEWER';
            const isOwner = role === 'OWNER';
            const isCurrentUser = currentUser && ((userEmail && currentUser.email && currentUser.email.toLowerCase() === userEmail.toLowerCase()) || (userId && currentUser.id && currentUser.id === userId));
            const canManage = isWorkspaceAdmin(activeWorkspace) && !isOwner && !isCurrentUser;
            const roleMeta = {
                OWNER: { label: 'Owner', icon: 'crown', color: '#2563eb' },
                ADMIN: { label: 'Admin', icon: 'shield-check', color: '#8b5cf6' },
                EDITOR: { label: 'Editor', icon: 'edit-3', color: '#0284c7' },
                VIEWER: { label: 'Viewer', icon: 'eye', color: '#64748b' },
                COMMENTER: { label: 'Commenter', icon: 'message-circle', color: '#d97706' }
            }[role] || { label: 'Viewer', icon: 'eye', color: '#64748b' };
            const initial = (userName.charAt(0) || 'U').toUpperCase();
            const control = canManage
                ? `<select class="access-role-select" aria-label="Role for ${escapeHtml(userName)}" onchange="changeMemberRole(${activeWorkspace.id}, ${userId}, this.value)">
                        <option value="ADMIN" ${role === 'ADMIN' ? 'selected' : ''}>Admin</option>
                        <option value="EDITOR" ${role === 'EDITOR' ? 'selected' : ''}>Editor</option>
                        <option value="VIEWER" ${role === 'VIEWER' ? 'selected' : ''}>Viewer</option>
                    </select>
                    <button type="button" class="btn btn-icon access-remove-btn" onclick="removeWorkspaceMember(${userId})" title="Remove ${escapeHtml(userName)}" aria-label="Remove ${escapeHtml(userName)}"><i data-lucide="user-minus" style="width:15px;height:15px;"></i></button>`
                : `<span class="access-role-badge" style="--role-color:${roleMeta.color};"><i data-lucide="${roleMeta.icon}" style="width:13px;height:13px;"></i>${roleMeta.label}</span>`;

            return `<div class="ws-member-item">
                <div class="access-member-identity">
                    <div class="access-member-avatar" style="--avatar-color:${roleMeta.color};">${initial}</div>
                    <div class="access-member-copy">
                        <div class="access-member-name">${escapeHtml(userName)} ${isCurrentUser ? '<span class="access-you-badge">You</span>' : ''}</div>
                        <div class="access-member-email">${escapeHtml(userEmail || 'No email provided')}</div>
                    </div>
                </div>
                <div class="access-member-actions">${control}</div>
            </div>`;
        }
