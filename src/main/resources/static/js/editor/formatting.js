        ...
        function initCollaborativeImageDragging() {
            const sheet = document.getElementById('docPageSheet');
            if (!sheet || sheet.dataset.imageDragReady === 'true') return;
            sheet.dataset.imageDragReady = 'true';

            sheet.addEventListener('mousedown', (event) => {
                const image = event.target.closest('img');
                if (!image || !sheet.contains(image) || event.button !== 0) return;
                event.preventDefault();
                event.stopPropagation();
                ...
                const left = Math.round(imageRect.left - sheetRect.left + sheet.scrollLeft);
                const top = Math.round(imageRect.top - sheetRect.top + sheet.scrollTop);
                ...
                image.dataset.collabImage = 'true';
                image.dataset.imageX = String(left);
                image.dataset.imageY = String(top);

                activeImageDrag = { image, sheet, offsetX, offsetY };
                document.addEventListener('mousemove', moveCollaborativeImage);
                document.addEventListener('mouseup', endCollaborativeImageDrag, { once: true });
            });
        }

        function moveCollaborativeImage(event) {
            if (!activeImageDrag) return;
            const { image, sheet, offsetX, offsetY } = activeImageDrag;
            const sheetRect = sheet.getBoundingClientRect();
            const left = Math.max(0, Math.round(event.clientX - sheetRect.left + sheet.scrollLeft - offsetX));
            const top = Math.max(0, Math.round(event.clientY - sheetRect.top + sheet.scrollTop - offsetY));
            image.style.left = `${left}px`;
            image.style.top = `${top}px`;
            image.dataset.imageX = String(left);
            image.dataset.imageY = String(top);
            ...
            if (typeof window.broadcastDocumentEdit === 'function') {
                window.broadcastDocumentEdit(sheet.innerHTML, window.currentDoc?.title || '');
            }
        }

        function endCollaborativeImageDrag() {
            if (!activeImageDrag) return;
            const { image, sheet } = activeImageDrag;
            image.style.cursor = 'grab';
            activeImageDrag = null;
            document.removeEventListener('mousemove', moveCollaborativeImage);
            if (typeof window.onDocChange === 'function') window.onDocChange();
            if (typeof window.updateDocStats === 'function') window.updateDocStats();
        }

        document.addEventListener('DOMContentLoaded', initCollaborativeImageDragging);
