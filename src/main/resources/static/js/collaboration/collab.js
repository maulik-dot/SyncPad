        ...
        let documentUsesRichMarkup = false;
        let collaborationInteractionListenersAttached = false;
        ...
        function getLocalPointerCoordinates(event) {
            const sheet = document.getElementById('docPageSheet');
            if (!sheet || !event || !sheet.contains(event.target)) return null;
            const rect = sheet.getBoundingClientRect();
            return {
                left: Math.max(0, Math.round(event.clientX - rect.left)),
                top: Math.max(0, Math.round(event.clientY - rect.top)),
                height: 20
            };
        }

        function broadcastLocalCursorPosition(immediate = false, pointerEvent = null) {
            if (!stompClient || !stompClient.connected || !currentDoc || !currentDoc.id || !currentUser) return;
            if (isReceivingRemoteEdit) return;

            const send = () => {
                const coords = getLocalCaretCoordinates() || lastPointerCoordinates;
                if (!coords) return;
        ...
        function subscribeToDocumentCollaboration(docId) {
            ...
            documentUsesRichMarkup = !!(currentDoc && /<[a-z][^>]*>/i.test(currentDoc.content || ''));
            attachCollaborationInteractionListeners();
        ...
        function attachCollaborationInteractionListeners() {
            if (collaborationInteractionListenersAttached) return;
            collaborationInteractionListenersAttached = true;

            document.addEventListener('selectionchange', () => broadcastLocalCursorPosition());
            document.addEventListener('keyup', () => broadcastLocalCursorPosition(true));
            document.addEventListener('mouseup', () => broadcastLocalCursorPosition(true));
            document.addEventListener('mousemove', (event) => {
                const sheet = document.getElementById('docPageSheet');
                if (!sheet || !sheet.contains(event.target)) return;
                lastPointerCoordinates = getLocalPointerCoordinates(event);
                broadcastLocalCursorPosition(false, event);
            }, { passive: true });
        }
        ...
        function handleRemoteCrdtOperation(op) {
            ...
            if (documentUsesRichMarkup) return;
        ...
        function broadcastDocumentEdit(content, title) {
            ...
            documentUsesRichMarkup = /<[a-z][^>]*>/i.test(content || '');

            // CRDT is reserved for plain text. Rich documents travel as HTML so
            // lists, embeds, code blocks, callouts, formatting, and AI output survive.
            if (window.SyncPadCrdt && !documentUsesRichMarkup) {
                try {
        ...
        function handleDocumentCollaborationEvent(data) {
            ...
            if (data.type === 'CURSOR') {
                ...
                if (sheet && data.content !== undefined) {
                    const isFocused = (document.activeElement === sheet || sheet.contains(document.activeElement));
                    const containsRichMarkup = /<[a-z][^>]*>/i.test(data.content);
                    // When CRDT is active and local user is focused/typing, character-level CRDT operations
                    // already handle plain text convergence without clobbering the caret. Rich formatting,
                    // lists, blocks, and embedded elements still arrive through the HTML edit broadcast.
                    if (!isFocused || !window.SyncPadCrdt || containsRichMarkup) {
                        const savedOffset = isFocused ? getCaretCharacterOffsetWithin(sheet) : -1;

                        isReceivingRemoteEdit = true;
                        sheet.innerHTML = data.content;
                        currentDoc.content = data.content;
                        documentUsesRichMarkup = /<[a-z][^>]*>/i.test(data.content || '');
                        if (window.SyncPadCrdt) {
                            try {
