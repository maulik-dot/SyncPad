...
class SyncPadLatexEngine {
        ...
        for (let line of lines) {
            ...
            for (const part of parts) {
                if (part.startsWith('$') && part.endsWith('$') && part.length > 2) {
                    ...
                    if (typeof window.katex !== 'undefined' && typeof window.katex.renderToString === 'function') {
                        try {
                            formattedLine += window.katex.renderToString(formula, { displayMode: false, throwOnError: false });
                            continue;
                        } catch (e) {}
                    }
                    formattedLine += `<span class="math-fallback-inline">${this.fallbackParseLatex(formula)}</span>`;
                } else {
    ...
    openHoverEditor(cardOrId, event) {
        ...
        const isInline = card.classList && card.classList.contains('doc-latex-inline');
        if (isInline) {
            this.activeInlineEl = card;
            this.activeHoverCardId = null;
        } else {
            this.activeInlineEl = null;
            this.activeHoverCardId = card.id || ('latex_' + Date.now());
            if (!card.id) card.id = this.activeHoverCardId;
        }
        const source = card.getAttribute('data-latex-source') || '';
        const currentSize = card.getAttribute('data-latex-size') || (isInline ? '1.1rem' : '1.2rem');
