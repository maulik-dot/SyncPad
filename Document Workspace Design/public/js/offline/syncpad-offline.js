...
class SyncPadOfflineDB {
  ...
  async cacheDocument(doc) {
    return this.saveDocumentLocally(doc);
  }

  async saveDocumentOffline(docId, title, content) {
    const doc = {
      id: Number(docId),
      title: title,
      content: content,
    ...
    await this.saveDocumentLocally(doc);
    await this.queueMutation({
      url: `/documents/${docId}`,
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + (localStorage.getItem('syncpad_token') || '')
      },
      body: { title, content }
    });
  }
      ...
      for (const item of queue) {
        try {
          ...
          if (!res.ok && res.status !== 404 && res.status !== 409) {
            console.warn('Server rejected offline mutation with status:', res.status);
            return;
          }
        } catch (err) {
...
window.SyncPadOffline = window.syncPadOffline;
