...
(function () {
    ...
    class SyncPadCrdtEngine {
        ...
        applyInsert(id, leftOriginId, rightOriginId, value) {
            ...
            while (insertIndex < this.characters.length) {
        ...
        /**
         * Reconciles CRDT character sequence with a new text/HTML string using
         * prefix/suffix diffing and returns generated INSERT and DELETE operations.
         */
        syncFromText(newContent, senderEmail) {
            newContent = newContent || '';
            const active = this.getActiveCharacters();
            const currentText = active.map(c => c.value).join('');
            if (currentText === newContent) return [];

            const ops = [];
            let prefixLen = 0;
            const minLen = Math.min(currentText.length, newContent.length);
            while (prefixLen < minLen && currentText[prefixLen] === newContent[prefixLen]) {
                prefixLen++;
            }
            ...
            while (
                ...
                currentText[currentText.length - 1 - suffixLen] === newContent[newContent.length - 1 - suffixLen]
            ) {
                suffixLen++;
            }
            ...
            const insertStr = newContent.substring(prefixLen, newContent.length - suffixLen);
            for (let i = 0; i < insertStr.length; i++) {
                const charVal = insertStr.charAt(i);
                const insOp = this.localInsert(prefixLen + i, charVal, senderEmail);
                if (insOp) ops.push(insOp);
            }
