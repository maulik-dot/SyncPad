...
(function () {
    ...
    window.executeCodeAi = function (actionType) {
        console.log('[ code-latex-ai ] executeCodeAi clicked:', actionType, 'ctx:', window._codeLatexContext);
        if (typeof toast === 'function') toast(`AI ${actionType} clicked — generating...`);
        const ctx = window._codeLatexContext;
        if (!ctx) {
            console.warn('[ code-latex-ai ] no _codeLatexContext, fallback to executeAiAction');
