...
<body>
    ...
    static sanitizeSheetDOM(sheetEl) {
        ...
        // 2. Remove editor-only transient UI controls but preserve indent styles
        ...
        // Ensure indented blocks keep margin-left for export (was set via style)
        // No stripping of margin-left — preserved as inline style

        // 3. Ensure LaTeX math cards have compiled KaTeX math and are block-level for print
        clone.querySelectorAll('.doc-latex-card').forEach(card => {
            ...
            // Keep floating cards absolute — preserve exact pixel position & size for PDF
            // Ensure they remain is-floating with left/top in px/pt and size
            if (card.classList.contains('is-floating') || card.getAttribute('data-latex-floating') === 'true') {
                // Keep absolute, ensure zIndex and size are preserved
                card.style.position = 'absolute';
                card.style.zIndex = '10';
                card.style.margin = '0';
                card.classList.add('is-floating');
                if (mathWrap) {
                    const size = card.getAttribute('data-latex-size') || mathWrap.style.fontSize || '1.2rem';
                    mathWrap.style.fontSize = size;
                }
            }
        });
    ...
    static renderBlock(block) {
        ...
        switch (block.type) {
            ...
            case 'code': {
                ...
                const codeIndent = block.indentPt && block.indentPt > 0 ? ` style="margin-left: ${block.indentPt}pt"` : '';
                return `<div class="doc-code-card"${codeIndent}>
            ...
            case 'heading': {
                ...
                let hStyles = [];
                if (block.align && block.align !== 'left') hStyles.push(`text-align: ${block.align}`);
                if (block.indentPt && block.indentPt > 0) hStyles.push(`margin-left: ${block.indentPt}pt`);
                const hStyleAttr = hStyles.length ? ` style="${hStyles.join('; ')}"` : '';
                return `<${tag}${hStyleAttr}>${runsHTML}</${tag}>`;
            }

            case 'paragraph': {
                const runsHTML = PdfExportRenderer.renderRuns(block.runs);
                let pStyles = [];
                if (block.align && block.align !== 'left') pStyles.push(`text-align: ${block.align}`);
                if (block.lineSpacing && Math.abs(block.lineSpacing - 1.65) > 0.05) pStyles.push(`line-height: ${block.lineSpacing}`);
                if (block.spacingBeforePt) pStyles.push(`margin-top: ${block.spacingBeforePt}pt`);
                if (block.spacingAfterPt && block.spacingAfterPt !== 6) pStyles.push(`margin-bottom: ${block.spacingAfterPt}pt`);
                if (block.indentPt && block.indentPt > 0) pStyles.push(`margin-left: ${block.indentPt}pt`);
                const pStyleAttr = pStyles.length ? ` style="${pStyles.join('; ')}"` : '';
                return `<p${pStyleAttr}>${runsHTML || '&nbsp;'}</p>`;
            }
            ...
            case 'table': {
                const tableIndent = block.indentPt && block.indentPt > 0 ? ` style="margin-left: ${block.indentPt}pt"` : '';
                let tableHTML = `<table class="doc-table"${tableIndent}><tbody>`;
            ...
            case 'callout': {
                ...
                let calloutStyles = [];
                if (block.accentColor) calloutStyles.push(`border-left-color: ${block.accentColor} !important`);
                if (block.backgroundColor) calloutStyles.push(`background-color: ${block.backgroundColor} !important`);
                if (block.indentPt && block.indentPt > 0) calloutStyles.push(`margin-left: ${block.indentPt}pt`);
                const styleAttr = calloutStyles.length ? ` style="${calloutStyles.join('; ')}"` : '';
            ...
            case 'blockquote': {
                const runsHTML = PdfExportRenderer.renderRuns(block.runs);
                const bqStyle = block.indentPt && block.indentPt > 0 ? ` style="margin-left: ${block.indentPt}pt"` : '';
                return `<blockquote${bqStyle}>${runsHTML}</blockquote>`;
            }
            ...
            case 'latex': {
                ...
                if (block.isFloating) {
                    // Use computed px for pixel-perfect size if available, else rem
                    let sizeStyle = '';
                    if (block.sizePx && block.sizePx.includes('px')) {
                        sizeStyle = `font-size: ${block.sizePx};`;
                    } else if (block.sizeRem) {
                        sizeStyle = `font-size: ${block.sizeRem};`;
                    } else {
                        sizeStyle = 'font-size: 1.2rem;';
                    }
                    // Preserve exact position: prefer px for 1:1, fallback to pt
                    let posStyle = '';
                    if (Number.isFinite(block.leftPx) && Number.isFinite(block.topPx)) {
                        posStyle = `left: ${block.leftPx}px; top: ${block.topPx}px;`;
                    } else {
                        const left = Number.isFinite(block.leftPt) ? block.leftPt : 0;
                        const top = Number.isFinite(block.topPt) ? block.topPt : 0;
                        posStyle = `left: ${left}pt; top: ${top}pt;`;
                    }
                    return `<div class="doc-latex-card is-floating" style="position: absolute; ${posStyle} ${sizeStyle} z-index: 10; background: transparent; border: none; box-shadow: none; margin: 0;"><div class="latex-rendered-math" style="${sizeStyle}">${mathHtml}</div></div>`;
                }

                let blockSize = '';
                if (block.sizePx && block.sizePx.includes('px')) blockSize = ` style="font-size: ${block.sizePx}"`;
                else if (block.sizeRem) blockSize = ` style="font-size: ${block.sizeRem}"`;
                return `<div class="doc-latex-card"><div class="latex-rendered-math"${blockSize}>${mathHtml}</div></div>`;
            }
    ...
    static renderRuns(runs) {
        ...
        for (const run of runs) {
            ...
            if (run.gradient && run.gradient.includes('gradient')) {
                styles.push(`background: ${run.gradient};`);
            ...
            } else if (run.color && run.color !== '#1e293b' && run.color !== '#0f172a') {
                styles.push(`color: ${run.color};`);
            } else if (run.color && run.color === '#1e293b' && run.font) {
                styles.push(`color: ${run.color};`);
            }
            if (run.font) {
            ...
            if (run.subscript) styles.push(`vertical-align: sub; font-size: 0.75em;`);
            if (run.superscript) styles.push(`vertical-align: super; font-size: 0.75em;`);
