...
class DocumentModel {
    ...
    static parseNode(node, rootSheet) {
        ...
        // 3. Headings (H1 - H6) — preserves alignment and indent
        if (/^h[1-6]$/.test(tagName)) {
            ...
            const headingLineH = computed.lineHeight && computed.lineHeight !== 'normal' ? parseFloat(computed.lineHeight) / (parseFloat(computed.fontSize)||16) : (level===1?1.3:1.35);
            const headingIndentPt = DocumentModel.parsePxToPt(computed.marginLeft) || 0;

            return {
                type: 'heading',
                level: level,
                align: align,
                indentPt: headingIndentPt,
        ...
        if (tagName === 'pre' || el.classList.contains('doc-code-block') || el.classList.contains('doc-code-card') || el.classList.contains('code-block') || (tagName === 'div' && el.querySelector('pre'))) {
            ...
            const codeIndentPt = DocumentModel.parsePxToPt(computed.marginLeft) || 0;
            return {
                type: 'code',
                language: lang,
        ...
        if (el.classList.contains('doc-latex-card') || el.classList.contains('latex-block') || el.getAttribute('data-latex') || el.getAttribute('data-latex-source')) {
            ...
            let sizeStr = el.getAttribute('data-latex-size') || el.style.fontSize || '1.2rem';
            // Capture computed font-size for pixel-perfect export (rem → px)
            try {
                const mathElForSize = el.querySelector('.latex-rendered-math');
                if (mathElForSize) {
                    const compSize = window.getComputedStyle(mathElForSize).fontSize;
                    if (compSize && compSize.includes('px')) {
                        // Store both rem and computed px for fidelity
                        el.setAttribute('data-latex-computed-px', compSize);
                    }
                }
            } catch(e) {}

            // Prefer computed px for exact size, fallback to rem
            let computedPx = el.getAttribute('data-latex-computed-px');
            if (!computedPx) {
                try {
                    const m = el.querySelector('.latex-rendered-math');
                    if (m) computedPx = window.getComputedStyle(m).fontSize;
                } catch(e) {}
            }
            return {
                type: 'latex',
        ...
        if (el.classList.contains('doc-callout') || el.classList.contains('callout') || (computed.borderLeftWidth && parseInt(computed.borderLeftWidth) >= 3 && computed.backgroundColor && computed.backgroundColor !== 'rgba(0, 0, 0, 0)')) {
            ...
            const calloutIndentPt = DocumentModel.parsePxToPt(computed.marginLeft) || 0;
        ...
        // 6. Blockquote / Quote — preserves indent
        if (tagName === 'blockquote' || el.classList.contains('doc-quote')) {
            const quoteIndentPt = DocumentModel.parsePxToPt(computed.marginLeft) || 0;
            return {
                type: 'blockquote',
                align: computed.textAlign || 'left',
        ...
        if (tagName === 'table') {
            ...
            const tableIndentPt = DocumentModel.parsePxToPt(computed.marginLeft) || 0;
            return {
                type: 'table',
                colCount: colCount,
        ...
        // 12. Generic Paragraphs & DIV blocks — now preserves line-height, paragraph spacing and indent
        const align = computed.textAlign || 'left';
        const rawLineH = computed.lineHeight;
        let lineSpacing = 1.65;
        if (rawLineH && rawLineH !== 'normal') {
            const lh = parseFloat(rawLineH);
            const fs = parseFloat(computed.fontSize) || 16;
            if (!isNaN(lh) && !isNaN(fs) && fs > 0) {
                lineSpacing = rawLineH.includes('px') ? (lh / fs) : lh;
                lineSpacing = Math.min(Math.max(lineSpacing, 1.0), 3.0);
            }
        }
        const marginTopPt = DocumentModel.parsePxToPt(computed.marginTop) || 0;
        const marginBottomPt = DocumentModel.parsePxToPt(computed.marginBottom) || 6;
        const marginLeftPt = DocumentModel.parsePxToPt(computed.marginLeft) || 0;
        const indentLevel = Math.round(marginLeftPt / 30); // 40px ~30pt per level
        ...
        return {
            ...
            align: align,
            lineSpacing: Math.min(Math.max(lineSpacing, 1.0), 3.0),
            spacingBeforePt: marginTopPt,
            spacingAfterPt: marginBottomPt,
            indentPt: marginLeftPt,
    ...
    static extractTextRuns(rootEl, inherited = {}) {
        ...
        function walk(node, currentStyle) {
                    ...
                    for (let i = 0; i < lines.length; i++) {
                        ...
                        if (line.length > 0) {
                            runs.push({
                                ...
                                subscript: Boolean(currentStyle.subscript),
                                superscript: Boolean(currentStyle.superscript),
                                color: currentStyle.color || '#1e293b',
                                highlight: currentStyle.highlight || null,
                                gradient: currentStyle.gradient || null,
            ...
            if (tag === 'sub') { nextStyle.subscript = true; nextStyle.superscript = false; }
            if (tag === 'sup') { nextStyle.superscript = true; nextStyle.subscript = false; }
            // Vertical align based computed style
            if (comp.verticalAlign === 'sub') { nextStyle.subscript = true; }
            if (comp.verticalAlign === 'super') { nextStyle.superscript = true; }
            ...
            // Detect gradient via computed backgroundImage
            if (comp.backgroundImage && comp.backgroundImage.includes('gradient')) {
                nextStyle.gradient = comp.backgroundImage;
                nextStyle.color = 'transparent';
                nextStyle.gradientClip = true;
            }
            if (tag === 'a' && el.href) {
            ...
            // Gradient text handling (background: linear-gradient + backgroundClip:text)
            if (el.style.background && el.style.background.includes('gradient')) {
                nextStyle.gradient = el.style.background;
                nextStyle.color = 'transparent';
            } else if (el.style.backgroundImage && el.style.backgroundImage.includes('gradient')) {
                nextStyle.gradient = el.style.backgroundImage;
                nextStyle.color = 'transparent';
            }
            if (el.style.webkitBackgroundClip === 'text' || el.style.backgroundClip === 'text') {
                nextStyle.gradientClip = true;
            }
